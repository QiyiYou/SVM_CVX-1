...
%% generate problem
num = 500;
% linear nonseparable condition
[dataset, label] = data_linear_nonseparable(num);
% general condition
% [dataset, label] = data_rbf(num);
%% kernel matrix
sigma = 30;
...
[sup_vec, w, b] = param_rbf(alfa, dataset, label, num, sigma);
%% plot
figure,
% judge every pixel
marg = 110;
...
for i = 1 : marg
    for j = 1 : marg - 10
        class(i, j) = 0;
        for m = 1 : num
            class(i, j) = class(i, j) + alfa(m) * label(m) * kernel_rbf([j, i], dataset(m, :), sigma);
        end
        if (class(i, j) + b > 0)
            class(i, j) = 1;
        else
