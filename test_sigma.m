...
for sigma_index = 1 : 3
    ...
    for i = 1 : ksize
        for j = 1 : ksize
            K(i, j) = kernel_rbf(trainset(i, :), trainset(j, :), sigma(sigma_index));
        end
    end
    %% CVX toolbox
    C = 10;
    ...
    [sup_vec, w, b] = param_rbf(alfa, trainset, label, num, sigma(sigma_index));
    ...
    for i = 1 : marg
        for j = 1 : marg - 10
            class(i, j) = 0;
            for m = 1 : num
                class(i, j) = class(i, j) + alfa(m) * label(m) * kernel_rbf([j, i], trainset(m, :), sigma(sigma_index));
            end
            if (class(i, j) + b > 0)
                class(i, j) = 1;
            else
    ...
    title ('test result with different sigma');
    xlabel('X');
    ylabel('Y');
end